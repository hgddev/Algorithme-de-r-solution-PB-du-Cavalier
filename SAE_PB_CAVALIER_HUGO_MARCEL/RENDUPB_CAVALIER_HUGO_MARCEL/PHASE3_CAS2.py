...
def backtracking(x, y, move_count):
    ...
    if move_count == TAILLE * TAILLE:
        return (x, y) in tab_position_finale
...
def coups_depart(x,y):
    ...
    for dx,dy in directions:
        ...
        if 0<=nx<TAILLE and 0<=ny<TAILLE:
            coups.append((nx,ny))
    return coups
#ici comme pour coups_possibles on enregistre les positions a 1 mouvement du point de départ pour que le cavalier revienne sur une des positions enregistrées
#mais cette fois on enregistre seulement les mouvements possibles non joués depuis le point de départ
#c'est pour cela que  coups_depart est différent de coups_possibles, coups_depart enregistre les positions a 1 mouvement du point de départ alors que coups_possibles enregistre les positions a 1 mouvement de la position actuelle du cavalier
tab_position_finale = coups_depart(start_x,start_y)


print("Calcul avec backtracking(8x8) : VEUILLEZ PATIENTER")

if backtracking(start_x, start_y, 1):
    print("CIRCUIT HAMILTONIEN TROUVE !")
    for ligne in echiquier:
        print(" ".join(f"{val:2d}" for val in ligne))
else:
    print("Aucun circuit n'a été trouvé")
